  ...
  if (req.method === 'GET') {
    try {
      const result = await pool.request().query(`
        SELECT ProductID, p_name, Inv_quantity, prod_type, date_add, supp, cost, url_link
        FROM [dbo].[PRODUCT]
      `);
    ...
    try {
      await pool.request()
        ...
        .query(`
          INSERT INTO [dbo].[PRODUCT] (ProductID, p_name, Inv_quantity, prod_type, date_add, supp, cost, url_link)
          VALUES (@ProductID, @p_name, @Inv_quantity, @prod_type, @date_add, @supp, @cost, @url_link)
        `);
